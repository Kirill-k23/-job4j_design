...
public class CSVReader {
    ...
    private static void print(String outType, List<String> strings) {
    ...
    public static void validate(ArgsName args) {
        ...
        if (!Files.isRegularFile(path)) {
            throw new IllegalArgumentException(String.format("%s isn't a file", path.toAbsolutePath()));
        }
    ...
    public static void main(String[] args) throws Exception {
        if (args.length < 4) {
            throw new IllegalArgumentException("Must be 4 arguments");
        }
